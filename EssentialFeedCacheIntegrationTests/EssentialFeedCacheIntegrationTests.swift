...
//
//  Created by David Gänshirt on 04.12.23.
//
...
final class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_load_deliversItemsSavedOnASperateInstance() {
        let sutToPerformSave = makeSUT()
        let sutToPerformLoad = makeSUT()
        let feed = uniqueImageFeed().models
        
        let saveExp = expectation(description: "Wait for save expectation")
        sutToPerformSave.save(feed) { saveError in
            XCTAssertNil(saveError, "Expected to save feed completion")
            saveExp.fulfill()
        }
